...
function parseCharacterSection(text) {
  ...
  if (an_set.size !== anointment_names.length)
    throw new Error("Duplicate anointment.");

  if (anointment_names.length > 20) throw new Error("Too many anointments.");
  if (spec !== "Royal" && anointment_names.length > 5)
    throw new Error("Too many anointments.");
...
function parseCreatureSection(text) {
  ...
  if (creatures.length > 7)
    throw new Error("Party appears to have more than 6 creatures.");
  ...
  if (!atLeastOne)
    throw new Error("The party string does not appear to contain any traits.");
...
function parsePartyString(text) {
  if (text.indexOf("========== CREATURES ==========") === -1) {
    throw new Error(
      "The string does not appear to be a valid Siralim Ultimate build string.",
    );
  }
